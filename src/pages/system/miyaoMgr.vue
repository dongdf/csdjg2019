<template>
    <div>
      <Row style="border-bottom:2px solid #EEE; padding-bottom: 10px;">
        <Col span="18">
        <div class="cpageTitle"><Icon size="20" type="md-key" /><span>秘钥管理</span></div>
   ...
   <br>
      ...
      <div v-if="active == 1">
        <Row class="rheader">
          <Col span="8" class="tleft">公司名称</Col>

          <Col span="8"  class="tcenter">使用时间</Col>
          <Col span="8" class="tcenter">数量</Col>
        </Row>
        <Row class="rerow" v-for="(o,idx) in odrList" :key="idx">
          <Col span="8" class="tleft"><Icon size="15" style="opacity: .6; position:relative; top:0px"  type="md-briefcase" />{{o.companyName}}</Col>

          <Col span="8"  class="tcenter">{{o.useTime | timeNumb('yyyy-MM-dd hh:mm')}}</Col>
          <Col span="8" class="tcenter"><span>-{{o.keytNum}}</span></Col>
        </Row>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      getused(){
        this.$http.post('financial/userService/useKeytList',{
          subData:{
            pageNum:this.pnumb,
            pageSize:this.psize
          }
